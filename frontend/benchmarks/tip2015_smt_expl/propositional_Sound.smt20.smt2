...
(declare-fun bar= (Int list) list4)
(declare-fun bar=2 (list Form) Bool)
(declare-fun formula (Form list2) Bool)
...
(assert (forall ((x Int)) (= (bar= x nil) nil4)))
(assert
  (forall ((x Int) (x2 list) (y2 Int))
    (= (bar= x (cons (pair22 y2 false) x2)) (bar= x x2))))
(assert
  (forall ((x Int) (x2 list) (y2 Int))
    (= (bar= x (cons (pair22 y2 true) x2))
      (cons4 (= x y2) (bar= x x2)))))
(assert
  (forall ((x list) (p Form) (q Form))
    (= (bar=2 x (|:&:| p q)) (and (bar=2 x p) (bar=2 x q)))))
(assert
  (forall ((x list) (r Form))
    (= (bar=2 x (Not r)) (not (bar=2 x r)))))
(assert
  (forall ((x list) (z Int)) (= (bar=2 x (Var z)) (or2 (bar= z x)))))
(assert (forall ((p Form)) (formula p nil2)))
(assert
  (forall ((p Form) (y list) (xs list2))
    (= (formula p (cons2 y xs)) (and (bar=2 y p) (formula p xs)))))
(assert
